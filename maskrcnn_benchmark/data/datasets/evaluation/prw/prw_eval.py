...
def do_prw_evaluation(datasets, predictions, output_folder, logger):
    ###########################################################
    tq_flag = False
...
def eval_detection_sysu(pred_boxlists, gt_boxlists, iou_thresh=0.5, use_07_metric=False):
    ...
    prec, rec = calc_detection_sysu_prec_rec(pred_boxlists=pred_boxlists, gt_boxlists=gt_boxlists, iou_thresh=iou_thresh)
...
def eval_reid_sysu(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, iou_thresh=0.5, use_07_metric=False):
    """Evaluate on voc dataset.
    Args:
        pred_boxlists(list[BoxList]): pred boxlist, has labels and scores fields.
        gt_boxlists(list[BoxList]): ground truth boxlist, has labels field.
    ...
    topk = [1, 3, 5, 10]
    #CMC = []
    real_result, ideal_result = calc_reid_sysu_topk(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, topk, det_thresh=0.5, iou_thresh=0.5)
